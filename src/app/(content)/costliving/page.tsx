...
export default function CostLiving() {
  ...
  const fetchDataAndCalculate = async () => {
    if (isFullInfo) {
      const fromPlace = PCIdata.stateData.find((place) => place.State === valueFromPlace);
      const toPlace = PCIdata.stateData.find((place) => place.State === valueToPlace);
      ...
      if (fromPlace && toPlace) {
        ...
        const fixedAssetFrom = _fromPlace?.["Tài sản"]?.["Tài sản cố định"] || {};
        const fixedAssetTo = _toPlace?.["Tài sản"]?.["Tài sản cố định"] || {};

        const assetFrom = _fromPlace?.["Tài sản"]?.["Tài sản không cố định"] || {};
        ...
        const fixedAssetPercentageChange = calculateDifference(fixedAssetFrom,fixedAssetTo);
        //total expensive asset
        const assetPercentageChange = calculateDifference(assetFrom, assetTo);
        //total expensive service
        const servicePercentChange = calculateDifference( serviceFrom,serviceTo);
  ...
  return (
    <div className={`${css.cover} flex flex-col items-center text-center`}>
      <div className="max-w-screen-xl flex flex-col items-center my-20">
        ...
        Nhập thông tin hai thành phố bạn muốn so sánh. Bạn có thể so sánh chi phí sinh hoạt ngay lập tức.
        </p>
      </div>
            ...
            <p className="px-0 lg:px-16 ">
              ...
              {difference < 0 ? (
                <div className="inline-flex items-center">
                  <span className="text-green-600 font-bold text-xl"> giảm {Math.abs(difference)}%</span>
                  <Icons.arrow_down width="2rem" height="1.5rem" />
                </div>
              ) : (
                <div className="inline-flex items-center">
                  <span className="text-red-600 font-bold text-xl"> tăng {difference}%</span>
                  <Icons.arrow_up width="2rem" height="1.5rem" />
                </div>
              )}
            </p>
          ) : null}
